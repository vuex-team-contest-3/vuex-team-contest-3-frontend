...
const updatedData = reactive({ data, image: "", imageURL: "" });
...
const updateClinic = async () => {
  ...
  formData.append("address", updatedData.data.address);
  formData.append("phone", updatedData.data.phone);
  if (updatedData.image) {
    formData.append("image", updatedData.image);
  }

  await useClinics
    .UPDATE(updateId.value, formData)
    .then((res) => {
  ...
  <div>
    ...
    >
      <i
        ...
        class="text-lg bg-red-500 px-2 p-1 absolute bx bx-trash -top-2 -right-2 rounded-full"
      ></i>
      <i
        v-if="isUser == 0 && !updateId"
        @click="() => (updateId = data.id)"
        class="text-lg bg-green-500 px-2 p-1 absolute bx bx-pencil -top-2 -left-2 rounded-full"
      ></i>
      <router-link :to="router" class="block">
        <img
          v-if="data.image_name"
          :src="`https://nest-clinic.onrender.com/api/image/${data.image_name}`"
          class="mb-5 h-32 w-32 mx-auto bg-zinc-900 border border-zinc-700 group-hover:border-zinc-600 duration-300 rounded-full object-contain"
        />
        <img
          v-else
          src="@/assets/logo.png"
        ...
        <div v-if="isUser == 1" class="flex items-center justify-between pt-2">
          <div class="flex items-center justify-between gap-2">
            <i
              class="text-red-500 bx bxs-heart text-xl hover:scale-125 duration-150"
            ></i>
