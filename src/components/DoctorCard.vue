    ...
    <div v-if="isAdmin" class="w-full flex items-end justify-between pt-5">
      ...
      <div class="flex items-center gap-2">
        <i
          ...
          class="text-lg bg-green-500 px-1 bx bx-pencil rounded-lg cursor-pointer"
        ></i>
        <i
          @click="() => (deleteId = data.id)"
          class="text-lg bg-red-500 px-1 bx bx-trash rounded-lg cursor-pointer"
        ></i>
      </div>
    </div>
  </div>
...
<script setup>
const props = defineProps(["doctor", "idAdmin"]);
</script>
